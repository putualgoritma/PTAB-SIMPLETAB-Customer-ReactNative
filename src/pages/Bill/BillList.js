...
const BillList = ({navigation, route}) => {
  ...
  const ctmPayAPi = () => {
    Promise.all([
      API.ctmpay(data.code, TOKEN),
      API.ctmcustomer(data.code, TOKEN),
    ])
      .then(result => {
        let data = [];
        var tunggakan = 0;
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        ...
        <View style={{paddingLeft: 10, flex: 1}}>
          <Title title="Info Tagihan Pelanggan" />
          ...
          <DataView
            title="Status"
            txt={customer.status == 1 ? 'Aktif' : 'Pasif'}
          />
        ...
        <View style={{paddingLeft: 10}}>
          <Text style={{fontSize: 19, color: '#696969', fontWeight: 'bold'}}>
            Jumlah Tagihan
          </Text>
        ...
        <View>
          <Text
            ...
            Note/Catatan: Pembayaran dengan transfer dari bank lain ke BPD Bali,
            tambahkan tagihan diatas dengan biaya administrasi sebesar Rp.
            2.000.,-saat memasukkan nominal transaksi
          </Text>
        </View>
      </ScrollView>
      <Footer focus="Menu" navigation={navigation} />
    </View>
  );
};
